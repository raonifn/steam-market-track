(function ($) {
    ...
    this.handleHtmlListings = function (html, callback) {
      ...
      $(listingsHtml).each(function () {
        ...
        linkToRemove = linkToRemove.replace(
          /javascript:RemoveMarketListing\('mylisting', '\d+', (\d+), '(\d+)', '(\d+)'\)/, "$1,$2,$3");
          linkToRemove = linkToRemove.split(',');
        listing.asset = {};
        listing.asset.appid = linkToRemove[0];
          listing.asset.contextid = linkToRemove[1] + '';
          listing.asset.id = linkToRemove[2];
          

        var lnk = $(this).find('a.market_listing_item_name_link').attr('href');
        lnk = lnk.replace(/\?.*$/, '');

        Product.search(lnk, 10, self.search_ajax_manager, function (product) {
  ...
  Trader.prototype.buy = function (listing, callback) {
    var url = 'https://steamcommunity.com/market/buylisting/' + listing.listingid;
    var data = {
      sessionid: getCookie('sessionid'),
      currency: g_rgWalletInfo['wallet_currency'],
  ...
  Trader.prototype.start = function() {
    this.search_ajax_manager.start();
    this.ajax_manager.start();
  };

  Trader.prototype.stop = function() {
    this.search_ajax_manager.stop();
