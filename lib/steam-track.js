(function($) {
   ...
   Seller.prototype.sell = function(listing, price) {
      ...
      self.messageTranscation("Buy", listing, listing.total_price);
      ...
      this.ajax_manager.ajax({
         ...
         success : function(xhr) {
            ...
            self.messageTranscation("Sell", this.listing, this.price);
         },
         data : data
      });
   };
   ...
   Seller.prototype.undercut = function(product, my_listing) {
      ...
      this.removeListing(my_listing, function(listing) {
         this.sell(listing, (min.subtotal_price * 100) - 1);
      });
   };
   ...
   Seller.prototype.removeListing = function(listing, callback) {
      this.ajax_manager.ajax({
         ...
         success : function(xhr) {
            ...
            self.messageTranscation("Removed", this.listing);
            if (callback) {
               callback(listing);
            }
   ...
   Seller.prototype.messageTranscation = function(type, listing, price) {
   ...
   Tracker.prototype.undercuteAllListings = function() {
      var self = this;
      this.searchListings(function(listings_x_product) {
         keys(listings_x_product).each(function(listing) {
            var product = listings_x_product[listing];
   ...
   Tracker.prototype.searchListings = function(callback) {
      this.undercut_ajax_manager.ajax({
         ...
         success : function(data) {
            if (callback) {
               var listings_x_product = this.tracker.handleHtmlListings(data.results_html);
               callback(listings_x_product);
            }
         }
      });
   };

   Tracker.prototype.handleHtmlListings = function(html) {
      var self = this;
      var listings_x_products = {};
      ...
      $(listingsHtml).each(function() {
         var id = $(this).attr('id');
         id = id.replace(/mylisting_(\d*)_name/, "$1");
         ...
         lnk = lnk.replace(/\?.*$/, '');
         self.searchProducts(lnk, 10, self.undercut_ajax_manager, function(productUrl, html) {
            var product = new Product(html, productUrl);

            listing.url = productUrl;
            listings_x_products[listing, product];
         });
      });

      return listings_x_products;
   };
   ...
   Tracker.prototype.handleHtmlList = function(data) {
      ...
      $(links).each(function(index) {
         ...
         self.searchProducts(lnk, 10, self.ajax_manager, function(product, dat) {
   ...
   Tracker.prototype.searchProducts = function(product, count, ajax_manager, callback) {
      ...
      ajax_manager.ajax({
         ...
         success : function(data) {
            if (callback) {
               callback(this.product, data.results_html);
            }
         }
      });
   };
