(function($) {
   ...
   Product = function(result_html) {
      this.extractData(result_html);
   };

   Product.prototype.extractData = function(result_html) {
      ...
      this.diffPriceWithoutFee = Number((diff - (avg * 0.15)).toFixed(2));
      this.url = product;
   ...
   Tracker.prototype.wantBuy = function(product) {
      var result = product.minPrice / product.avgPrice;

      var passedThreshold = result <= this.threshold;
   ...
   Tracker.prototype.init = function() {
      ...
      buttonCleanQueue.click(function() {
         ...
         self.ajax_manager.mutex = ajax_manager.max;
      });
      ...
      form.append($('<input type="text" id="max_ajax" value="' + this.ajaxManager.max + '" />'));
   ...
   Tracker.prototype.list = function(page) {
      ...
      this.ajax_manager.ajax({
         ...
         success : function(data) {
            if (start == 0) {
               var pages = (data.total_count / self.search_param.pagesize) + 1;
               for (var i = 1; i <= pages; i++) {
                  self.list(i);
               }
            }
            this.list = false;
            self.handleHtmlList(data.results_html);
         }
      });
   };

   Tracker.prototype.handleHtml = function(product, data) {
      if (!this.ajax_manager.timer) {
         return;
      }
      var product = new Product(data.results_html);

      if (tracker.debug) {
         console.info('product', product);
      }
      if (product.result <= this.threshold && product.diffWithFee >= this.minDiff) {
   ...
   Tracker.prototype.search = function(product) {
      var self = this;
   ...
   AjaxManager.prototype.start = function() {
      ajax_manager.timer = setInterval(this.consume, this.schedule_time);
   };
   ...
   AjaxManager.prototype.consume = function consume() {
   ...
   BlaBle = function() {
      console.info('eeee', this);
   };

   BlaBle.prototype.doIt = function() {
      console.info('aaaa', this);
   };
